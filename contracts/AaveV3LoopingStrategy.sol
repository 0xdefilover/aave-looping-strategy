...
contract AaveV3LoopingStrategy is IAaveV3LoopingStrategy, ReentrancyGuard {
    ...
    function repayDebtWithCollateral(RepayDebtWithCollateralParams calldata params) external nonReentrant {
        ...
        uint callerDebt = IDebtToken(params.borrPool).balanceOf(msg.sender);
        uint borrAmt = params.debtAmt > callerDebt ? callerDebt : params.debtAmt;

        // note: use balance difference to calculate real vaule to emit event
        uint callerCollBalBf = IERC20(params.collPool).balanceOf(msg.sender);
        uint callerDebtBalBf = IERC20(params.borrPool).balanceOf(msg.sender);
    ...
    function _increasePos(IncreasePosParams calldata params) internal {
        ...
        // note: use balance difference to calculate real vaule to emit event
        uint callerCollBalBf = IERC20(params.collPool).balanceOf(msg.sender);
        uint callerDebtBalBf = IDebtToken(params.borrPool).balanceOf(msg.sender);
    ...
    function _decreasePos(DecreasePosParams memory params) internal returns (uint amtOut) {
        // validate tokenOut
        address borrToken = IDebtToken(params.borrPool).UNDERLYING_ASSET_ADDRESS();
        address collToken = IAToken(params.collPool).UNDERLYING_ASSET_ADDRESS();
    ...
    function _flashLoan(
        ...
        amts[0] = borrAmt;
        uint[] memory interestModes = new uint[](1);
    ...
    function _executeOperationIncreasePos(FlashLoanParams memory params) internal {
        ...
        uint supplyAmt = IERC20(collToken).balanceOf(address(this));
    ...
    function _executeOperationDecreasePos(
        ...
        // cases:
        // 1 tokenOut is borrPool's underlying token -> swap exact in coll pool's underlying token to borr pool's underlying token
        ...
        // 2 tokenOut is collPool's underlying token -> swap exact out coll pool's underlying token to borr pool's underlying token
        else {
    ...
    function _swapExactIn(address swapper, address from, address to, uint amtIn, uint minAmtOut, bytes memory data)
        internal
    {
        ISwapper.SwapExactInParams memory swapExactInParams =
            ISwapper.SwapExactInParams({from: from, to: to, data: data});
        ...
        uint amtOut = IERC20(to).balanceOf(address(this));
        require(amtOut >= minAmtOut, 'AaveV3LoopingStrategy: insufficient amtOut');
        emit Swap(swapper, from, to, amtIn, amtOut);
    }

    function _swapExactOut(address swapper, address from, address to, uint amtOut, uint maxAmtIn, bytes memory data)
        internal
    {
        ...
        // transfer token to swapper
        uint tokenInBalBf = IERC20(from).balanceOf(address(this));
        ...
        uint amtIn = tokenInBalBf - IERC20(from).balanceOf(address(this));
    ...
    function _reducePos(address collPool, uint collAmt, address borrToken, uint repayAmt, address onBehalfOf)
        internal
        returns (uint)
    {
        // repay debt
        ...
        return IPool(POOL).withdraw(IAToken(collPool).UNDERLYING_ASSET_ADDRESS(), type(uint).max, address(this));
    }
